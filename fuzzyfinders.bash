...
if [ -z "$FUZZYFINDER" ]; then
  if command -v sk >/dev/null 2>&1; then
    FUZZYFINDER="sk --reverse --tiebreak=index,score"
    [ -n "$SKIM_DEFAULT_OPTIONS" ] && FUZZYFINDER="$FUZZYFINDER $SKIM_DEFAULT_OPTIONS"
...
# __fuzzyfinder_man_list_all() {
#   local parent dir file
#   IFS=':' read -r -a paths <<< "$(man --all --location)"
#   for parent in $paths; do
#     for dir in $(/bin/ls -1 "$parent"); do
#       local p="${parent}/${dir}"
#       printf %s\\n "${lines[@]}"
#       if [ -d "$p" ]; then
#         IFS=$'\n' read -r -a lines <<< "$(/bin/ls -1 "$p")"
...
#
# fuzzyfinder-man() {
#   local selected=$(__fuzzyfinder-man_list_all | eval "$FUZZYFINDER --prompt 'man >'")
#   if [ -n "$selected" ]; then
#     man "$selected"
#   fi
# }
